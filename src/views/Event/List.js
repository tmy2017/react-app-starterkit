    ...
    if (errors.length && errors[0] === "Invalid alias") {
      return (
        <div className="alert alert-warning">
          You have not yet created an Event Resource in your project. Either
          create an Event Resource with the path of "event" or import the
          src/project.json into your project.
        </div>
      );
    }
    ...
    onAction: (submission, action) => {
      switch (action) {
        ...
        case "edit":
          dispatch(
            push(
              `/form/${ownProps.match.params.formId}/submission/${
                submission._id
              }/edit`
            )
          );
